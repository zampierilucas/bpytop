...
class GpuBox(Box):
	...
	def _draw_fg(cls):
		...
		for gpu in collector.gpus:
			...
			stat_nums = collector.stat_nums[gpu.uuid]
			...
			# power
			out += f'{Mv.to(y+len(stat_nums["freqs"])-2, x)}'
...
class GpuCollector(Collector):
	...
	def _get_stat_nums(cls, card):
		...
		if "AMD" in card.brand:
			...
			cls.stat_nums[card.uuid]["fans"] = cls._get_fans(card)
			cls.stat_nums[card.uuid]["freqs"] = cls._get_freqs(card)
			cls.stat_nums[card.uuid]["power"] = cls._get_power(card)
			cls.stat_nums[card.uuid]["volts"] = cls._get_volts(card)
			cls.stat_nums[card.uuid]["temps"] = cls._get_vitals(card)
			cls.stat_nums[card.uuid]["load"] = cls._get_load(card)
		else:
	...
	def _get_power(cls, card):
		...
		if "AMD" in card.brand:
			...
			try:
				power = nvml.nvmlDeviceGetPowerUsage(card.card) / 1000
			except:
				return NotImplemented
		else:
			errlog.debug("GPU not supported")

		for i in cls.stat_nums[card.uuid]["power"]:
	...
	def _collect(cls):
		...
		errlog.debug(f"len {len(cls.gpus)}")
